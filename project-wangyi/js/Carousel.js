/*
 *	轮播图组件api说明：
 * 		1、依赖move.js，组件前要引入move.js
 * 		2、轮播图需要有一个父级作为容器，并且需要有id；
 * 		3、添加自定义事件调用函数on(type，listener)，type为自定义事件名： leftEnd/rightEnd、leftClick/rightClick
 * 		4、listener为事件监听函数；
 */

;(function(window,undefined){

    //构造函数Carousel
    var Carousel=function(){
        this.settings={             //配置参数说明
            id:'pic',				//轮播图父级的id，必需传的参数
            autoplay:true,			//自动播放，true为自动，false为不自动，默认为true
            intervalTime:1000,		//间隔时间，运动后停顿的时间，默认为1s;
            loop:true,				//循环播放，true为循环，fasle为不循环，默认为true
            totalNum:5,				//图片总量
            moveNum:1,				//单次运动的图片数量(图片数量必须是运动数量的倍数)
            circle:true,			//小圆点功能，true为显示，false为不显示，默认显示
            moveWay:'opacity'		//运动方式，opacity为透明度过渡，position为位置过渡
        }
    };

    Carousel.prototype={

        constructor:Carousel,

        init:function(opt){
            var opt=opt||this.settings;

            for(var attr in opt){
                this.settings[attr]=opt[attr];
            };
            this.createDom();
        },

        createDom:function(){

            //创建上一个按钮
            this.prevBtn=document.createElement('div');
            this.prevBtn.className='prev';
            this.prevBtn.innerHTML='&lsaquo;';
            var This=this;
            this.prevBtn.onclick=function(){
                This.prev();
                This.trigger('leftClick');
            };

            //获取上一个按钮的父级
            this.box=document.getElementById(this.settings.id);
            this.box.appendChild(this.prevBtn);


            //创建下一个按钮
            this.nextBtn=document.createElement('div');
            this.nextBtn.className='next';
            this.nextBtn.innerHTML='&rsaquo;';
            this.nextBtn.onclick=function(){
                This.next();
                This.trigger('rightClick');
            }
            this.box.appendChild(this.nextBtn);

            //创建小圆点，取决于是否有小圆点的参数
            this.circleWrap=document.createElement('div');
            this.circleWrap.className='circle';
            this.circles=[];            //用来存放圆点，方便后面对于圆点的操作


            for(var i=0;i<this.settings.totalNum/this.settings.moveNum;i++){
                var span=document.createElement('span');
                span.index=i;

                span.onclick=function(){
                    This.cn=this.index;
                    if(This.settings.moveWay=='opacity'){
                        This.opacityFn();
                        This.ln=This.cn;
                    }else{
                        This.positionFn();
                    }
                    ev.cancelBubble=true;
                };

                this.circleWrap.appendChild(span);
                this.circles.push(span);
            };

            this.circles[0].className='active';
            if(this.settings.circle){
                this.box.appendChild(this.circleWrap);
            };

            this.moveInit();
        },

        moveInit:function(){
            this.cn=0;              //当前的索引
            this.ln=0;              //上一个索引
            this.canClick=true;     //是否可以再次点击
            this.endNum=this.settings.totalNum/this.settings.moveNum;           //停止条件
            this.opacityItem=this.box.children[0].children;             //运动透明度的元素
            this.positionItemWrap=this.box.children[0].children[0];     //运动位置的元素的父级
            this.positionItem=this.positionItemWrap.children;           //运动位置的所有的元素

            switch(this.settings.moveWay){

                case 'opacity':         //如果走的是透明度，需要设置透明度和transition
                    for(var i=0;i<this.opacityItem.length;i++){
                        this.opacityItem[i].style.opacity=0;
                        this.opacityItem[i].style.transition='1s opacity';
                    }
                    this.opacityItem[0].style.opacity=1;
                    break;

                case 'position':        //如果走的是位置，需要设置父级的宽度
                    //这里需要注意：要加上元素的margin
                    var leftMargin=parseInt(getComputedStyle(this.positionItem[0]).marginLeft);
                    var rightMargin=parseInt(getComputedStyle(this.positionItem[0]).marginRight);

                    //一个运动元素的实际高度
                    this.singleWidth=leftMargin+this.positionItem[0].offsetWidth+rightMargin;

                    //如果运动是循环的，就需要复制一份内容
                    if(this.settings.loop){
                        this.positionItemWrap.innerHTML+=this.positionItemWrap.innerHTML;
                    }

                    //复制内容后才能设置宽度
                    this.positionItemWrap.style.width=this.singleWidth*this.positionItem.length+'px';
            }

            if(this.settings.autoplay){
                this.autoplay();
            }
        },

        opacityFn:function(){               //透明度运动方式

            //左边到头
            if(this.cn<0){
                if(this.settings.loop){
                    this.cn=this.endNum-1;
                }else{
                    this.cn=0;
                    this.canClick=true;
                }
            }

            //右边到头
            if(this.cn>this.endNum-1){
                if(this.settings.loop){
                    this.cn=0;
                }else{
                    this.cn=this.endNum-1;
                    this.canClick=true;
                }
            }

            this.opacityItem[this.ln].style.opacity=0;
            this.circles[this.ln].className='';

            //当前图片的样式
            this.opacityItem[this.cn].style.opacity=1;
            this.circles[this.cn].className='active';

            var en=0;
            var This=this;
            this.opacityItem[this.cn].addEventListener('transitionend',function(){
                en++;
                if(en==1){
                    This.canClick=true;
                    This.ln=This.cn;
                }

                This.endFn();               //调用自定义事件
            })
        },

        positionFn:function(){

            //修改圆点，只有不循环的时候才去修改圆点
            /*if(!this.settings.loop){
                this.circles[this.ln].className='';
                this.circles[this.cn].className='active';
            }*/

            //左边到头
            if(this.cn<0){
                if(this.settings.loop){
                    //这里是循环，需要做两件事：
                    /*
                     *  1、先让运动的父级的位置往左走一个内容的位置(未复制的内容)；
                     *  2、同时修改索引值；
                     */
                    this.positionItemWrap.style.left=-this.positionItemWrap.offsetWidth/2+'px';
                    this.cn=this.endNum-1;
                }else{
                    this.cn=0;
                }
            }
            console.log(this.cn);

            //右边到头
            if(this.cn>this.endNum/2 && !this.settings.loop){
                this.cn=this.endNum/2;
            }else{
                //这里不需要做任何事情，需要在运动结束后做条件判断；
            }

            var This=this;
            //运动，需要用到move函数；left=一个元素的宽度*当前cn的值*一次运动元素的个数
            move(
                this.positionItemWrap,
                {left:-this.cn*this.singleWidth*this.settings.moveNum},
                300,
                'linear',
                function(){

                    //当走到第二份的第一屏的时候就需要让父级的left为0；
                    if(This.cn==This.endNum){
                        This.positionItemWrap.style.left=0;
                        This.cn=0;
                    };

                    This.endFn();               //调用自定义事件

                    This.canClick=true;
                    This.ln=This.cn;
                }
            )
        },

        prev:function(){                    //上一个按钮点击功能
            //能否进行下一次点击，要放在这里进行判断
            if(!this.canClick){
                return;
            }
            this.cn--;
            this.canClick=false;
            if(this.settings.moveWay=='opacity'){
                this.opacityFn();
            }else{
                this.positionFn();
            }
        },

        next:function(){                    //下一个按钮点击功能
            if(!this.canClick){
                return;
            }
            this.cn++;
            this.canClick=false;
            if(this.settings.moveWay=='opacity'){
                this.opacityFn();
            }else{
                this.positionFn();
            }
        },

        autoplay:function(){
            var This=this;
            this.timer=setInterval(
                function(){
                    This.next();
                },
                this.settings.intervalTime
            );

            //鼠标放在box身上清除定时器
            this.box.onmouseenter=function(){
                clearInterval(This.timer);
                This.timer=null;
            }

            //鼠标离开box身上继续播放
            this.box.onmouseleave=function(){
                This.autoplay();
            }
        },

        //添加自定义事件
        on:function(type,listener){
            this.events=this.events || {};
            this.events[type]=this.events[type] || [];
            this.events[type].push(listener);
        },

        //触发自定义事件
        trigger:function(type){

            //因为有的组件有自定义事件，有的没有，所以需要做一个判断，只有有调用自定义事件的实例才能执行下面的代码
            if(this.events && this.events[type]){
                for(var i=0;i<this.events[type].length;i++){
                    this.events[type][i].call(this)
                }
            }
        },

        endFn:function(){
            //统一添加自定义事件的函数，要在运动完成以后添加，并且需要加给不循环的运动
            if(!this.settings.loop){

                //这个条件满足说明左边的运动已经到头了；
                if(this.cn==0){
                    this.trigger('leftEnd');
                };

                //这个条件满足说明右边的运动已经到头了；
                if(this.cn==this.endNum/2){
                    this.trigger('rightEnd');
                };
            }
        }
    }

    window.Carousel=Carousel;

})(window,undefined)